-- Script CORRETTO per risolvere il timeout della query corsi
-- Sostituisci tutto il contenuto del SQL Editor con questo
...
-- 6. Test ORDER BY separato (SENZA COUNT)
...
-- 8. Test finale della query originale (CORRETTA)
SELECT 'Test query originale' as test, COUNT(*) as count 
FROM corsi 
WHERE is_active = true;

-- 9. Test ORDER BY separato per la query originale
SELECT 'Test ORDER BY originale' as test, id, title, is_active, created_at 
FROM corsi 
WHERE is_active = true
ORDER BY created_at DESC 
LIMIT 5;
